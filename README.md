...
│   │   ├──索引
│   │   │  └──目的：减少磁盘I/O次数，加快搜索时间。
│   │   │  └──优点：保证数据唯一性，加速排序和分组；缺点：创建需要消耗资源，可能会失效。
│   │   │  └──实现方式:hash表，BST树，AVL树，红黑树，B/B+树。
│   │   │  └──主键索引:一张表只能有一个，不能为null，且唯一。
│   │   │  └──二级索引:除了主键索引外的所有索引。
│   │   │  │   └──聚簇索引与非聚簇索引：区别在于索引数据是否与数据存放在一起
│   │   │  │   └──覆盖索引和联合索引：覆盖索引包含所有需要查询的字段值；联合索引由表中多个字段创建。
│   │   │  └──索引失效场景
│   │   │  │  └──SQL写法破坏了B+树的二分查找特性。
│   │   │  │  └──优化器基于I/O成本妥协。
│   │   ├──日志
│   │   │  └──redo log（重做日志）
│   │   │  │  └──目的：保持数据的完整性，让MYSQL有崩溃恢复能力。
│   │   │  │  └──原理：数据写入时，写入到日志文件中，然后再写入到数据文件中。
│   │   │  │  └──刷盘时机，事务提交时；redo log buffer 空间不足时；后台线程周期性刷新等。
│   │   │  │  └──日志文件组：磁盘上存储的redo log日志文件是以日志文件组的形式组成的，采用环形数组。
│   │   │  └──binlog
│   │   │  │  └──binlog是逻辑日志，记录所有涉及更新数据的逻辑操做。
│   │   │  └──二阶段提交
│   │   │  │  └──将 redo log 的写入拆成了两个步骤prepare和commit，防止redo log 与 binlog 两份日志之间的逻辑不一致。
│   │   │  └──undo log（回滚日志）
│   │   │  │  └──undo log 属于逻辑日志,记录了数据被修改前的状态。
│   │   │  │  └──目的：事务回滚（保障原子性）；支撑 MVCC（多版本并发控制）；辅助崩溃恢复。
│   │   │  │  └──总结来说，Redo Log 负责“向前重做”以保证数据不丢，而 Undo Log 则负责“向后回退”以保证事务可撤销和并发读取。
│   ├── redis/                   #   数据结构 / 集群 / 分布式锁
